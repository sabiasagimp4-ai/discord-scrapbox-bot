...
毎日8:00（日本時間）に、Scrapbox・YouTube Data API・OpenRouterの疎通確認を自動実行します（Gyazoは対象外。`/status` では引き続き確認できます）。全て正常な場合は何も通知しませんが、いずれかが異常な場合のみ指定チャンネルに警告を送信します。`SCRAPBOX_SID` Cookieの期限切れなど、気づかないうちに保存が失敗し続ける事態を早期発見するための機能です。
...
**既存ページが大量に流れてこないための仕組み**（過去に発生した誤通知への対策）:
...
- DM本文の先頭を **`ページ:`** で始めると、日記への追記ではなく**新しいページの作成**になります（下記）
- 追記できたら✅、失敗したら❌のリアクションがDMメッセージに付きます（返信メッセージは送らず、DMを日記本文で汚しません）
- 複数行のメッセージを送ると、2行目以降はインデントされます

**新しいページを作る**

DM本文の先頭を **`ページ:`**（全角コロン `ページ：` も可）で始めると、日記への追記ではなく、**その日記プロジェクトに独立したページを新規作成**します。1行目の残りがタイトル、2行目以降が本文です。
...
```
Scrapbox: 「Blender Guru」ページ
Blender Guru
チュートリアルが分かりやすい
[https://www.youtube.com/@blenderguru]
```

- 同時に**その日の日記の【日記】欄に `[Blender Guru]` が1行追記**されます。リンクを張らないとどこからも辿れないページになってしまうため、日記が索引として機能する形を保っています
- **同名のページが既にある場合は上書きせず、末尾に追記**します（インポートAPIは渡した行でページ全体を置き換えるため、既存の本文を読み直してから足しています）
- 本文は日記と同じく[ページ名の自動リンク](#dmでの即追記)が働きます。作成したページ自体も直後から自動リンクの対象になります（ページ一覧のキャッシュを破棄するため）
- タイトルの `[ ]` は丸括弧に置き換えられます（Scrapboxはタイトルに角括弧を使えないため）
- 画像を添付すれば、そのページの本文に貼られます（日記側ではなく作成したページに入ります）
- `ページ:` の後ろが空だと❌が付き、`/status` の直近エラーに記録されます
- この機能は**DM限定**です。iOSショートカット（Webhook）経由では `ページ:` も通常の日記追記として扱われます

**写真を送る**

DMに画像を添付すると、**Gyazoにアップロードしてから**その日の日記に貼られます（本文なしで写真だけ送ってもOK）。

```
【日記】
 展示に行った
  [https://i.gyazo.com/xxxxxxxx.jpg]
```

- Discordの添付ファイルURLは時間が経つと失効するため、そのまま貼らずGyazoの恒久URLに変換します。**`GYAZO_ACCESS_TOKEN` が必要**です（未設定だと画像だけ追記されず、`/status` の直近エラーに記録されます）
- 本文と一緒に送ると、本文の下に画像行が続きます。写真だけ送った場合は画像行だけが追記されます
- 画像以外の添付（PDF等）は無視されます。Scrapboxに貼っても表示できないためです
- `単語:` で始まるDMに写真を付けた場合、単語は【新しく知った単語】欄に、写真は【日記】欄に分かれて入ります（写真は単語ではなくその日の記録のため）

**ページ名の自動リンク**

日記本文に**日記プロジェクト内の既存ページ名**が出てきたら、自動でScrapboxのリンク記法 `[ ]` に変換します。`単語:` で貯めた単語が、後日の日記から自動で繋がるようになります。

```
（Scrapboxに [serendipity] ページがある状態で）
DM:  今日はserendipityという言葉を思い出した
結果: 今日は[serendipity]という言葉を思い出した
```

- 大文字小文字は区別せず照合し、リンクにはページ側の表記を使います（`scrapbox` と書いても `[Scrapbox]` になります）
- 複数のページ名が重なる場合は**長い方**を優先します（`Blender` と `Blender Guru` があれば後者）
- 既に `[ ]` で囲まれている部分・URL・`#タグ` は変換対象外です（二重リンクやURLの破壊を防ぐため）
- 日付ページ（日記そのもの）と `日記` ページは、本文に出てきてもリンクにしません
- 1文字のページ名は無関係な文字に当たって誤リンクを量産するため対象外です
- ページ一覧は5分間キャッシュされます。新しく作ったページは最大5分リンクされないことがあります
- Webhook経由（後述のiOSショートカット）の追記にも同じように働きます
- **`DIARY_AUTOLINK=0` で無効化**できます。書いたままの文章を残したい場合はこちらを設定してください
...
DM追記と同じ3つ（`DIARY_SCRAPBOX_PROJECT`・`DIARY_SCRAPBOX_SID`・`DIARY_OWNER_USER_ID`）が設定されていると、毎日22:00（日本時間、`DIARY_REMINDER_HOUR` で変更可）にその日の日記ページを確認し、**まだ何も書かれていない場合だけ**自分宛にDMが届きます。

```
📔 2026-07-06 の日記がまだ空です。
今日のお題: 今日いちばん時間を使ったことは何でしたか？
このDMに返信すれば【日記】欄に、「単語:〇〇」なら【新しく知った単語】欄に追記されます。「ページ:〇〇」で新しいページも作れます。写真もそのまま送れます。
https://scrapbox.io/my-diary/2026-07-06
```

- **既に何か書いてある日は何も送りません**。毎日必ず届く通知は読み飛ばされるようになるため、催促は「本当に書いていない日」だけに限定しています
- 「今日はどうでしたか」だけだと書き出しに詰まるため、**その日のお題を1つ**添えます。お題は日付から決定的に選ばれるので、同じ日なら何度確認しても同じお題で、日をまたげば必ず別のお題になります。文面を変えたい場合は `diary.py` の `DIARY_PROMPTS` を編集してください
- 0:05に自動作成された雛形（`#日記`・ナビゲーション行・見出しだけの状態）は「書かれていない」と判定します。ページ自体がまだ無い場合も同じ扱いです
- 逆に、【日記】欄でも【新しく知った単語】欄でも、雛形以外の行が1行でもあれば「書いた」と判定します
- Scrapboxに接続できず確認できなかった場合はDMを送りません（空だと誤判定して、書いてある日に催促するのを避けるため）。失敗は `/status` の直近エラーに記録されます
- 届いたDMにそのまま返信すれば、上の「DMでの即追記」がそのまま働きます（Scrapboxを開く必要はありません）
- 実行結果（催促を送った／記入済みで送らなかった）は `/status` の稼働情報に「日記リマインド」として表示されます。日記の内容は表示されません
...
また、**角括弧 `[` `]` は丸括弧 `(` `)` に置き換えられます**。Scrapboxはリンク記法と衝突するためタイトルに `[ ]` を含められず、そのまま送ると `Title should not use bracket` という400エラーで保存に失敗します。YouTubeの動画名には `[Official Music Video]` のような角括弧が頻出するため、情報を落とさずに保存できるよう自動変換しています（例: `ChouCho - Defy the Silence [Lyric MV]` → `ChouCho - Defy the Silence (Lyric MV)`）。全角の `【 】` はScrapboxでも使えるのでそのまま残ります。`/write` で自分で入力したタイトルにも同じ正規化がかかります。
...
Scrapboxプロジェクト内の全ページから動画URLを探し、Windows上のEagleへ動画ファイルとして登録できます。通常はEagle内の`eagle-plugin/`を使います。BotはRender上、Eagleと動画ダウンロードは手元のPC上で動くため、EagleのローカルAPIを外部公開する必要はありません。

### 事前準備

1. Windows PCにEagle、`yt-dlp.exe`、`ffmpeg.exe`をインストールし、両方をPATHへ追加します。
2. Renderの環境変数に`EAGLE_BRIDGE_TOKEN`を設定します。十分に長いランダム文字列を使い、ログやDiscordへ貼らないでください。
3. `eagle-plugin/`をEagleのプラグインとしてインストールします。
...
3. Eagleの`Scrapbox Video Importer`を開き、Bot URLと`EAGLE_BRIDGE_TOKEN`を入力します。

4. 「全ページをスキャン」→プレビュー確認→「このプレビューを取り込む」→「取り込み開始」の順に操作します。

5. `/eagle status`またはプラグインの「状態更新」で待機・処理中・成功・失敗件数を確認できます。失敗した動画は`/eagle retry-failed`で再試行できます。

プラグインは動画を一度に1件ずつ処理し、成功した正規化URLをホームフォルダーの`/scrapbox-video-import-manifest.json`へ保存します。再実行時はEagleへ登録済みのURLをスキップします。従来のPython Bridgeを使う場合は、`eagle_bridge.py`の手順も利用できます。Renderの再起動でクラウド側の未完了キューが失われた場合は、`/eagle import-all`またはプラグインのスキャンから再実行してください。

### 制限事項

- YouTube Data API、Cookie、ログインセッションは使用しません。
- 公開動画が対象です。非公開、会員限定、年齢制限、地域制限、ログイン必須の動画は失敗する可能性があります。
- Eagleが起動していない、`yt-dlp`/`ffmpeg`が見つからない、ディスク容量が足りない場合は個別ジョブが失敗します。
...
| `EAGLE_BRIDGE_TOKEN` | — | EagleプラグインまたはWindows BridgeがジョブAPIへ接続するための認証トークン。未設定時はEagle APIを無効化 | `ランダムな長い文字列` |
...
├── eagle-plugin/                 # Eagle内で動く動画取り込みプラグイン
│   ├── manifest.json
│   ├── index.html
│   ├── plugin.js
│   └── README.md
