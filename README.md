...
### `/project create` スラッシュコマンド

`/project create name:<案件名>` で、案件用のページを雛形付きで作成します。雛形には「概要」「データ」「メモ・感想」のセクション見出しと `#案件` タグが含まれます。同名ページが既にある場合は作成せず、そのページへのリンクを返します。
...
`/note page:<ページ名>` でポップアップ（モーダル）が開き、入力したテキストを**既存ページの末尾に追記**します（`/write` が新規作成・上書きなのに対し、こちらは追記）。案件ページへの進捗データや感想の書き込みを想定しています。指定したページが存在しない場合は新規作成します。

追記は日付と記入者名付きで、以下の形式でページ末尾に追加されます:
